...
public class GridManagerClient implements GridManager {
    ...
    @Override
    public Iterator<Node> chainFilter(Iterator<Node> original, Node linkNode, Node node2, LinkNodeTypes returnType) {
    ...
    @Override
    public Iterator<Node> chainFilter(Node node1, Iterator<Node> original, Node node2, LinkNodeTypes returnType) {
        if (original instanceof NodeIteratorClient) {
            final String key = ((NodeIteratorClient) original).getKey();
            final IteratorResponse response = createClientConnection()
                    .addPathPart("/chainFilter")
    ...
    @Override
    public Iterator<Node> chainFilter(Node node1, Node linkNode, Iterator<Node> original, LinkNodeTypes returnType) {
        if (original instanceof NodeIteratorClient) {
            final String key = ((NodeIteratorClient) original).getKey();
            final IteratorResponse response = createClientConnection()
                    .addPathPart("/chainFilter")
